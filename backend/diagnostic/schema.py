"""MS1 → MS2 questionnaire-answer contract (the 31 flat keys).

Single source of truth for the keys MS1 writes into project_profiles.diagnostic_answers.
MS2 reads these raw keys directly (no transformation), so every key + enum here must
match the scorer exactly. Self-contained (stdlib only) so it drops into the backend.

`validate_answers` checks a submission; `build_diagnostic_answers` produces the flat
dict to persist, with optional keys defaulted so every key is always present for MS2.
"""
...
def build_diagnostic_answers(answers: dict[str, Any]) -> dict[str, Any]:
    """Produce the flat diagnostic_answers dict MS2 reads (every key present)."""
    out: dict[str, Any] = {}
    for spec in FIELD_SPECS:
